{
  ...
  parseRequirements = content:
    ...
    |> map (
      ...
      in
        if (elemAt parts 0) == "#!" && (elemAt parts 1) == "nix-shell"
        then parts |> lib.drop 2 |> lib.concatStringsSep " " |> parseNixShellShebang
        else if (elemAt parts 0) == "#?" && (elemAt parts 1) == "alias"
        then {aliases = parts |> lib.drop 1;}
        else null
    )
  ...
  wrapScript = {
    ...
    binPath =
      if lib.hasAttr "packages" requirements
      then deps2binPath requirements.packages
      else null;
